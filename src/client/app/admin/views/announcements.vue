	...
	<ui-card>
		...
		<section v-for="(announcement, i) in announcements" class="fit-top">
			...
			<ui-horizon-group class="fit-bottom">
				<ui-button :disabled="!$store.getters.isAdmin" @click="save()"><fa :icon="['far', 'save']"/> {{ $t('save') }}</ui-button>
				<ui-button :disabled="!$store.getters.isAdmin" @click="remove(i)"><fa :icon="['far', 'trash-alt']"/> {{ $t('remove') }}</ui-button>
			</ui-horizon-group>
		</section>
		<section>
			<ui-button :disabled="!$store.getters.isAdmin" @click="add"><fa :icon="faPlus"/> {{ $t('add') }}</ui-button>
		</section>
	</ui-card>
</div>
</template>
